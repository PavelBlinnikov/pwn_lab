...
```
Итого: мы можем получать адреса со стека, а также записывать по произвольному адресу произвольное значение (количество уже выведенных байт). Таким образом можно получить исполнение произвольного кода. Конкретная эксплуатация сложнее чем в случае переполнения буфера, а потому выходит за рамки данной лабораторной работы, однако о ней можно узнать в [этом](https://youtu.be/t1LH9D5cuK4) видеоролике.
